/* 
    samp-bank
    A fork of Weponz/Dynamic SQLite Banking System (SA-MP)
...
enum {
    BANK_EDIT_DIALOG = 1000,
    BANK_NAME_DIALOG,
    BANK_MENU_DIALOG,
    BANK_BALANCE_DIALOG,
    BANK_WITHDRAW_DIALOG,
    BANK_DEPOSIT_DIALOG,
    BANK_DEBT_DIALOG,
    BANK_LOANS_DIALOG,
    BANK_PAY_DIALOG
...
IsNumeric(const string[]) // #stopthestockabuse
{
...
GetXYBehindPlayer(playerid, &Float:x, &Float:y, Float:distance) // #stopthestockabuse
{
	new Float:a;
...
GetName(playerid) // #stopthestockabuse
{
...
GetFreeBankSlot() // #stopthestockabuse
{
	new slot = -1;

    for(new i = 0; i < MAX_BANKS; i++)
    {
        new query[128]; // Array unnecessarily massive.
        ...
        if(!db_num_rows(res))
        {
            db_free_result(res); // We weren't freeing the result here beforehand. That is a major issue.
            return i;
        }
...
IsPlayerInRangeOfBank(playerid, Float:distance) // #stopthestockabuse
{
    for(new i = 0; i < MAX_BANKS; i++)
    {
...
LoadBanks() // #stopthestockabuse
{
	new query[128]; // why the fuck does the query need to be 800 characters long?
    ...
    for(new i = 0; i < MAX_BANKS; i++)
	{
        ...
        if(db_num_rows(res))
        {
            db_get_field_assoc(res, "NAME", BankData[i][bank_name], 64);

            BankData[i][bank_extx] = db_get_field_assoc_float(res, "EXTX");
            BankData[i][bank_exty] = db_get_field_assoc_float(res, "EXTY");
            BankData[i][bank_extz] = db_get_field_assoc_float(res, "EXTZ");


            BankData[i][bank_spawnx] = db_get_field_assoc_float(res, "SPAWNX");
            BankData[i][bank_spawny] = db_get_field_assoc_float(res, "SPAWNY");
            BankData[i][bank_spawnz] = db_get_field_assoc_float(res, "SPAWNZ");
            BankData[i][bank_spawna] = db_get_field_assoc_float(res, "SPAWNA");

            BankData[i][bank_loans] = bool:db_get_field_assoc_int(res, "LOANS");
			...
			BankData[i][bank_label] = CreateDynamic3DTextLabel(BankData[i][bank_name], X11_WHITE, BankData[i][bank_extx], BankData[i][bank_exty], BankData[i][bank_extz], 10.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, 0, 0, -1, 10.0);
...
LoadPlayerAccount(playerid) // #stopthestockabuse
{
...
SavePlayerAccount(playerid) // #stopthestockabuse
{
	new query[128];
...
ShowBankMenu(playerid) // #stopthestockabuse
{
...
public OnScriptInit()
{
    if ((db = db_open(BANK_DATABASE)) == DB: 0) {
        return printf("[samp-bank] Connection to the database failed.");
    }

    db_query(db, "CREATE TABLE IF NOT EXISTS `BANKS` (`ID`, `NAME`, `EXTX`, `EXTY`, `EXTZ`, `SPAWNX`, `SPAWNY`, `SPAWNZ`, `SPAWNA`, `LOANS`)");
    db_query(db, "CREATE TABLE IF NOT EXISTS `ACCOUNTS` (`NAME`, `BALANCE`, `DEBT`)");
		...
		if(response)
		{
			if(listitem == 0)
			{
				...
				if(funds < rating)
				{
					...
					return SendClientMessage(playerid, X11_RED, string);
				}
...
YCMD:bankcommands(playerid, params[], help)
{
	if(!IsPlayerAdmin(playerid)) return SendClientMessage(playerid, X11_RED, "ERROR: You do not have access to this command.");
	SendClientMessage(playerid, X11_WHITE, "COMMANDS: /createbank, /deletebank, /editbank");
	return 1;
}

YCMD:createbank(playerid, params[], help)
{
	if(!IsPlayerAdmin(playerid)) return SendClientMessage(playerid, X11_RED, "ERROR: You do not have access to this command.");
	if(GetFreeBankSlot() == -1) return SendClientMessage(playerid, X11_RED, "ERROR: There are no free bank slots. Increase MAX_BANKS");
	if(IsPlayerInRangeOfBank(playerid, BANK_ZONE) != -1) return SendClientMessage(playerid, X11_RED, "ERROR: You are too close to another bank, choose another location.");
	...
	for(new i = 0; i < sizeof(sa_zones); i++)
	{
		if(pos[0] >= sa_zones[i][zone_area][0] && pos[0] <= sa_zones[i][zone_area][3] && pos[1] >= sa_zones[i][zone_area][1] && pos[1] <= sa_zones[i][zone_area][4])
		{
			...
			BankData[bankid][bank_label] = CreateDynamic3DTextLabel(name, X11_WHITE, BankData[bankid][bank_extx], BankData[bankid][bank_exty], BankData[bankid][bank_extz], 10.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, 0, 0, -1, 10.0);
            ...
            BankData[bankid][bank_exit] = CreateDynamicCP(BANK_EXIT_X, BANK_EXIT_Y, BANK_EXIT_Z, 1.0, bankid, -1, -1, 5.0, -1, 1);
		  	...
		  	format(query, sizeof(query), "INSERT INTO `BANKS` (`ID`, `NAME`, `EXTX`, `EXTY`, `EXTZ`, `SPAWNX`, `SPAWNY`, `SPAWNZ`, `SPAWNA`, `LOANS`) VALUES ('%d', '%q', '%f', '%f', '%f', '%f', '%f', '%f', '%f', '%d')", bankid, name, BankData[bankid][bank_extx], BankData[bankid][bank_exty], BankData[bankid][bank_extz], BankData[bankid][bank_spawnx], BankData[bankid][bank_spawny], BankData[bankid][bank_spawnz], BankData[bankid][bank_spawna], BankData[bankid][bank_loans]);
			db_free_result(db_query(db, query));
...
YCMD:deletebank(playerid, params[], help)
{
	if(!IsPlayerAdmin(playerid)) return SendClientMessage(playerid, X11_RED, "ERROR: You do not have access to this command.");
	
	new bankid = IsPlayerInRangeOfBank(playerid, 10.0);
	if(bankid == -1) return SendClientMessage(playerid, X11_RED, "ERROR: You must be next to a bank entrance to use this command.");
    ...
    // You don't free the result whenever db_num_rows isn't equal to 1. So that's what we shall do!

    db_free_result(res);
	return 1;
}

YCMD:editbank(playerid, params[], help)
{
	if(!IsPlayerAdmin(playerid)) return SendClientMessage(playerid, X11_RED, "ERROR: You do not have access to this command.");
    ...
    format(string, sizeof string, "{FFFFFF}Change Name: %s\n{FFFFFF}Toggle Loans: %s", BankData[bankid][bank_name], (BankData[bankid][bank_loans] ? (""WHITE"Enabled") : (""RED"Disabled")));
